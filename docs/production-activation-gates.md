...
| Organization-policy baseline | `1-org/org-policies` is the sole owner for key prohibition, public-access prevention, external-IP, contact-domain, and workload constraints | **Unknown:** evaluate each high-risk constraint with Policy Simulator/dry-run, inventory conflicts, rehearse lockout recovery, then apply the exact reviewed organization-level plan; defaults or manual policy are not evidence |
