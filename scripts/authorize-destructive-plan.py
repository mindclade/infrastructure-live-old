...
CHANGE_REFERENCE = re.compile(r"^(?:CHG|INC|SEC|DR)-[A-Z0-9][A-Z0-9-]{2,63}$", re.IGNORECASE)
...
def main() -> int:
    ...
    try:
        ...
        return 2
    if data.get("schema_version") != "1.0.0" or not isinstance(data.get("destructive"), bool):
    ...
    if args.event_name != "workflow_dispatch":
        errors.append("destructive plans may not auto-apply from a push; use protected workflow_dispatch")
    ...
    if not CHANGE_REFERENCE.fullmatch(args.change_reference.strip()):
        errors.append("change_reference must match CHG-, INC-, SEC-, or DR- followed by a tracked identifier")
