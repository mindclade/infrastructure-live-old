...
dependency "vpc" {
  ...
  mock_outputs = {
    ...
    network_self_link = { development = "projects/mc-development-net/global/networks/development" }
  }
...
dependency "gateway_vip" {
  ...
  mock_outputs                            = { addresses = { development = "10.16.33.10" } }
  mock_outputs_allowed_terraform_commands = ["plan", "validate", "init"]
}
  ...
  zones = {
    mindclade-ai-private = {
      ...
      records = { for hostname in include.envcommon.locals.hostnames["ai"] :
        replace(hostname, ".", "-") => {
          name    = trimsuffix(hostname, ".mindclade.ai")
          type    = "A"
          ttl     = 300
    ...
    mindclade-dev-private = {
      ...
      records = { for hostname in include.envcommon.locals.hostnames["developer"] :
        replace(hostname, ".", "-") => {
          name    = trimsuffix(hostname, ".mindclade.dev")
          type    = "A"
          ttl     = 300
          rrdatas = [dependency.gateway_vip.outputs.addresses[include.envcommon.locals.environment]]
        }
    ...
    mindclade-studio-private = {
      ...
      records = { for hostname in include.envcommon.locals.hostnames["studio"] :
        replace(hostname, ".", "-") => {
          name    = hostname == "mindclade.studio" ? "@" : trimsuffix(hostname, ".mindclade.studio")
          type    = "A"
          ttl     = 300
          rrdatas = [dependency.gateway_vip.outputs.addresses[include.envcommon.locals.environment]]
        }
