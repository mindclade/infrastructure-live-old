...
dependency "shared" {
  ...
  mock_outputs = {
    project_ids = {
      platform = "mc-production-platform"
    }
  }
...
dependency "vpc" {
  ...
  # a scalar would have been a production VPC quietly shared with production.
  mock_outputs = {
    network_self_link    = { production = "projects/mock/global/networks/mock-production-vpc" }
    subnetwork_names     = { production = "mock-production-nodes" }
    pods_range_names     = { production = "mock-production-pods" }
    services_range_names = { production = "mock-production-services" }
    host_project_ids     = { production = "mc-production-net" }
  }
  mock_outputs_allowed_terraform_commands = ["plan", "validate", "init"]
}
...
dependency "kms" {
  ...
  # makes "revoke production's access to production data" a single IAM change.
  mock_outputs = {
    crypto_key_ids = {
      gke = "projects/mock/locations/europe-west4/keyRings/mock-production/cryptoKeys/gke"
    }
  }
  mock_outputs_allowed_terraform_commands = ["plan", "validate", "init"]
}
...
dependency "node_identities" {
  ...
  mock_outputs = {
    service_accounts = {
      system_nodes = { email = "sa-system-nodes@mc-production-platform.iam.gserviceaccount.com" }
    }
  }
  mock_outputs_allowed_terraform_commands = ["plan", "validate", "init"]
}
...
inputs = {
  # `include.envcommon.locals.environment` rather than a literal "production" — that local
