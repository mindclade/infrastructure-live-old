...
dependency "observability" {
  ...
  mock_outputs = {
    project_id = "mc-development-observability"
  }
  mock_outputs_allowed_terraform_commands = ["plan", "validate", "init"]
}
...
dependency "shared" {
  ...
  mock_outputs = {
    project_ids = { ops = "mc-development-ops" }
  }
  mock_outputs_allowed_terraform_commands = ["plan", "validate", "init"]
}
...
inputs = {
  ...
  # ---------------------------------------------------------------------------------------
  # Development pages nobody. Every alert here lands in a channel that is read during working
  # hours, because the point of alerting in development is to catch a bad rule before it
  ...
  alert_policies = {
    ...
    # Admission denials. Expected in development, where binary-authorization runs DRYRUN —
    ...
    admission-denials-rising = {
      ...
      documentation = <<-EOT
        Development runs Binary Authorization in DRYRUN, so these are denials that were
  ...
  # SLOs
  # ---------------------------------------------------------------------------------------
  # Defined in development as well as production, deliberately. An SLO written for the first
  ...
  slos = {
    runtime-gateway-availability = {
      ...
      # Development targets two nines rather than production's four. A development SLO at
