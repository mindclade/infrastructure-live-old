...
dependency "gke" {
  ...
  mock_outputs = {
    ...
    cluster_id   = "projects/mc-development-platform/locations/europe-west4/clusters/mc-development"
  }
...
dependency "kms" {
  ...
  mock_outputs = {
    crypto_key_ids = {
      storage = "projects/mock/locations/europe-west4/keyRings/mock-development/cryptoKeys/storage"
    }
  }
  mock_outputs_allowed_terraform_commands = ["plan", "validate", "init"]
}
...
inputs = {
  ...
  gke_backup = {
    ...
    # Daily in development. Production takes hourly, which is configured in its own unit —
    ...
    retention = {
      # 30 days of daily backups in development. The number that matters is not the count but
