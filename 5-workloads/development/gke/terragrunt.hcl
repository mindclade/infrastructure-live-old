...
dependency "kms" {
  ...
  mock_outputs = {
    crypto_key_ids = {
      gke = "projects/mock/locations/us-central1/keyRings/mock-development/cryptoKeys/gke"
    }
  }
  mock_outputs_allowed_terraform_commands = ["plan", "validate", "init"]
}
...
dependency "node_identities" {
  ...
  mock_outputs = {
    service_accounts = {
      system_nodes = { email = "sa-system-nodes@mc-development-platform.iam.gserviceaccount.com" }
    }
  }
  mock_outputs_allowed_terraform_commands = ["plan", "validate", "init"]
}
