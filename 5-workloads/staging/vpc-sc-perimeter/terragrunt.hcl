...
terraform {
  source = "${include.root.locals.module_source_base}//service_perimeter?ref=${local.module_version}"
}
...
dependency "data" {
  config_path = "../../../4-projects/staging/data"
...
dependency "serving" {
  config_path = "../../../4-projects/staging/serving"
...
dependency "security" {
  config_path                             = "../../../4-projects/staging/security"
...
inputs = {
  ...
  perimeter = {
    ...
    # DRY RUN
    # ------------------------------------------------------------------------------------
    # Keep staging in dry-run until the development perimeter has been enforced and the
    # staging violation log is clean for the agreed observation window.
  ...
  ingress_policies = [
    {
      ...
      from = {
        identities = [
          ...
          "serviceAccount:${include.root.locals.account_vars.locals.infrastructure_live_service_accounts.staging}",
        ]
        identity_type        = null
