...
dependency "vpc" {
  ...
  # a scalar would have been a production VPC quietly shared with staging.
  mock_outputs = {
    network_self_link    = { staging = "projects/mock/global/networks/mock-staging-vpc" }
    subnetwork_names     = { staging = "mock-staging-nodes" }
    pods_range_names     = { staging = "mock-staging-pods" }
    services_range_names = { staging = "mock-staging-services" }
...
inputs = {
  # `include.envcommon.locals.environment` rather than a literal "staging" — that local
