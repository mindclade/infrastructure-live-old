...
dependency "psa" {
  ...
  mock_outputs = {
    allocated_ip_ranges = { staging = "mc-staging-psa" }
  }
  mock_outputs_allowed_terraform_commands = ["plan", "validate", "init"]
}
...
dependency "kms" {
  ...
  mock_outputs = {
    crypto_key_ids = {
      sql = "projects/mock/locations/us-central1/keyRings/mock-staging/cryptoKeys/sql"
    }
  }
  mock_outputs_allowed_terraform_commands = ["plan", "validate", "init"]
}
...
dependency "kms_dr" {
  ...
  mock_outputs = {
    crypto_key_ids = {
      sql = "projects/mock/locations/us-east4/keyRings/mock-staging-dr/cryptoKeys/sql"
    }
  }
  mock_outputs_allowed_terraform_commands = ["plan", "validate", "init"]
}
...
inputs = {
  ...
  # Staging sizing. Production overrides this; the shape of the config does not change.
